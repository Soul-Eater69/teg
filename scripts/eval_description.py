...
async def _eval_ticket(ticket_id, props, vs_list, *, catalogue, llm, judge, raw_budget, sem) -> list[dict]:
    async with sem:
...
async def main(args: argparse.Namespace) -> None:
    ...
    results = await asyncio.gather(*(
        _eval_ticket(t, condensed[t], gt_vs[t], catalogue=catalogue, llm=llm, judge=judge,
...
if __name__ == "__main__":
    ...
    p.add_argument("--judge-model", default="", help="stronger model for the judges (e.g. gpt-5-idp); "
                   "empty = judge with the generation model (self-judge, may be optimistic)")
