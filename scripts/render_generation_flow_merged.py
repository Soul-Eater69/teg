...
ax.text(50, 173, "Generation flow (merged) — Capabilities in ONE call for all Value Streams",
        ha="center", fontsize=13.5, weight="bold", color="#1F4E79")
...
hCAP = box(ax, 88, row2, 24, "Capabilities (L3)",
           "LLM — ALL VS in ONE call;\nmatch to governed L3", LLM, ts=9.4, badge="LLM · 1 call · all VS", wfs=7.5)
...
ax.text(50, ny-5.2,
        "• Capabilities is now ONE batched call covering every Value Stream's stages — not one call per VS.\n"
        "   Strict stage isolation + salvage keep each L3 under its true stage (0 mislink in the output).\n"
        "• Business Needs stays one call per Value Stream; Description is independent of the stages.\n"
        "• Sequential link: Business Needs and Capabilities both need the SELECTED STAGES, so they wait\n"
        "   for Stage Selection. Call count drops from 3 + 2N to 4 + N (Capabilities N -> 1).",
        ha="center", va="top", fontsize=8.6, color="#333", linespacing=1.45)

fig.savefig("/tmp/generation_flow_merged.png", dpi=150, bbox_inches="tight", facecolor="white")
print("wrote /tmp/generation_flow_merged.png")
