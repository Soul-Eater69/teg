...
def _aggregate(per_query: list[dict], broad: set[str], gt_by_ticket: dict[str, set]) -> dict:
    ...
    return {
        ...
        "gt_size": {**_stats([q["gt_size"] for q in per_query]),
                    "single_vs": sum(1 for q in per_query if q["gt_size"] == 1),
                    "multi_vs": sum(1 for q in per_query if q["gt_size"] >= 2),
                    "histogram": dict(sorted(Counter(q["gt_size"] for q in per_query).items()))},
