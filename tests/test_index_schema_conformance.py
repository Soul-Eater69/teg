...
def test_historical_index_doc_conforms_to_schema() -> None:
    # idp_teg_data is historical-only: the index holds the ER retrieval docs. The Value Stream
    # catalogue is NOT indexed here (it is governed gold data in Azure SQL), so the schema has no
    # `properties` / Value Stream fields and there is no VS index doc to conform-check.
