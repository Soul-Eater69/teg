...
The theme description (shared body + batched framing) and stage selection (one batched call
for all value streams) are both generated once per ticket and passed in. So per VS this only
runs business needs + L2/L3 capabilities in parallel on the value stream's selected stages.
The theme title is deterministic: "<ticket title> - <value stream name>".
"""
...
async def generate_theme_package(
    *,
    ...
    selected_stages: list[SelectedStage],
) -> ThemePackage:
